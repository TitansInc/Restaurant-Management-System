...
$('#Submit').click(function(){
		...
		else
		{
			Stype='CollectMyself';
		Database();
		alert('thanks for self delivery');
		Confirm_Delete('All');
		$('#date').val('');
		$('#time').val('');
		$('#address').val('');
...
function Items()
{
	...
	if(category=='All' )
	{
		...
		{
			subs=MName[i].substr(0,im_name.length);
			if(im_name==subs || im_name.toUpperCase()==subs)
			{
	...
	else if(category!='All')
	{
		...
		{
			subs=MName[i].substr(0,im_name.length);
			if((im_name==subs || im_name.toUpperCase()==subs) && category==CName[i])
			{
					for(j=1;j<=Rating[i];j++)
					{
						imgs+="<img class='rate' src=Images/Rating/rate.png />";
